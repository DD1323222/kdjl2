...
function createLeft(){
	...
	cf.id='cf1';
	cf.innerHTML = fightcHtmlText(bb[0])+'<br /><font color=#097603>'+parseInt(bb[1], 10)+'级</font>';
	...
	var enemyName=fightcHtmlText(gg[0]).replace("精英","<font color=blue>精英</font>");
	var enemyWx=fightcHtmlText(gg[2]);
	lot.innerHTML='<div style="width:31px;height:37px;z-index:4;top:0;left:0;position:absolute;background:url(../images/ui/newmap/dr02.gif);"><div style="z-index:4;top:7px;left:15px;position:absolute"><font color="#2A9E49" size="2.5"><b>'+enemyWx+'</b></font></div></div><div style="width:108px;height:15px;line-height:15px;z-index:5;top:0;left:31px;position:absolute;padding-left:0;padding-top:0;overflow:hidden;white-space:nowrap;">'+enemyName+'&nbsp;LV：'+parseInt(gg[1],10)+'</div><div style="width:108px;height:11px;z-index:5;top:15px;left:31px;position:absolute"></div>';
	$('fm').appendChild(lot);
	...
	jnfont.style.cssText='position:absolute;left:547px;top:150px;font-weight:bold;width:250px;height:30px;font-family:华文新魏;font-size:18px;color:yellow';
	jnfont.id='pfont';
	...
	var opt = {
		 ...
		 onSuccess: function(t){
				if(t.responseText == 0 || t.responseText=='') {
					fightcRequestFailed('战斗请求失败，请稍候重试！');
					return;
				}
...
function splits(str)
{
	...
	if(crit.length < 3)
	{
		fightcRequestFailed(str || '挑战战斗状态异常，请重新进入！', false);
		return;
	}
	str = crit[0];
	wx_type = crit[2];
	...
	if(tt.length < 5)
	{
		fightcRequestFailed(str || '挑战战斗数据读取失败，请重新进入！', false);
		return;
	}
...
function buff_div(para,val)
{
	...
	badge.style.cssText = 'display:inline-block;width:18px;height:18px;line-height:18px;margin-right:2px;text-align:center;color:#fff;font-size:12px;font-weight:bold;background-color:'+buffColor+';';
	badge.title = para+'增加:'+String(val);
	badge.appendChild(document.createTextNode(buffText));
	med_buff.appendChild(badge);
}
	...
	else if(n==2)	// ack set.
	{
		for(var i=0;i<=7; i++)
		{
			try{
				...
				toolshd.innerHTML+="<span onclick=\"window.parent.usejn='"+tt[9]+"';closetl();\" style='border:1px solid #ccc;cursor:hand;'>"+ fightcHtmlText(tt[0]) +"</span>";
			}catch(e){continue;}
		}
	}
	...
	else if(n==4) // skill
	{
		...
		for(var i=0;i<=7; i++)
		{
			try{
				var tt = bbjn[i];
				if ( typeof bbjn[i] == 'undefined') continue;
				toolshd.innerHTML+="<span onclick=\"Usejn("+tt[9]+");closetl();\" style='border:1px solid #ccc;cursor:pointer;'>"+ fightcHtmlText(tt[0]) +"</span>";
			}catch(e){continue;}
		}
	}
	else if(n==5) // help
	{
		...
		for(var i=0;i<=bbfzp.length; i++)
		{
			try{
				...
				toolshd.innerHTML+="<span onclick=\"UseYao("+tt[2]+");closetl();\" style='border:1px solid #ccc;cursor:hand;'>"+ fightcHtmlText(tt[0]) +"</span>";
			}catch(e){continue;}
		}
	}
	else if(n==6) // catch
	{
		...
		for(var i=0;i<=catcharr.length; i++)
		{
			try{
				...
				toolshd.innerHTML+="<span onclick=\"viewCatch("+tt[2]+");closetl();\" style='border:1px solid #ccc;cursor:hand;'>"+ fightcHtmlText(tt[0]) +"</span>";
			}catch(e){continue;}
		}
	}
	else if(n==7) // props.
