...
$err = 0;
$paiSellTransactionActive = false;
$paiSellLockedItemId = 0;

function paiSellShutdown()
{
	global $_pm, $paiSellTransactionActive, $paiSellLockedItemId;
	$error = error_get_last();
	if(!is_array($error) || !in_array($error['type'], array(E_ERROR, E_PARSE, E_CORE_ERROR, E_COMPILE_ERROR), true)) return;
	if($paiSellTransactionActive) $_pm['mysql']->query('ROLLBACK');
	$paiSellTransactionActive = false;
	if($paiSellLockedItemId > 0)
	{
		unLockItem($paiSellLockedItemId);
		$paiSellLockedItemId = 0;
	}
}
register_shutdown_function('paiSellShutdown');
...
else
{
	...
	*/
	...
	{   // 是否可交易检查
		...
		if(!$_pm['mysql']->query('START TRANSACTION')) { unLockItem($bid); $paiSellLockedItemId = 0; $_pm['mem']->memClose(); die('3'); }
		$paiSellTransactionActive = true;
