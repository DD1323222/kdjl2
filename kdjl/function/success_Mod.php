...
$db = &$_pm['mysql'];
$successPayTransactionActive = false;

function successPayAbort($message)
{
	global $db, $successPayTransactionActive;
	if($successPayTransactionActive)
	{
		$db->query('ROLLBACK');
		$successPayTransactionActive = false;
	}
...
function successPayShutdown()
{
	global $db, $successPayTransactionActive;
	if($successPayTransactionActive && isset($db))
	{
		$db->query('ROLLBACK');
		$successPayTransactionActive = false;
	}
...
register_shutdown_function('successPayShutdown');
...
if($notice){//支付成功后51post数据过来
	...
	*/
	...
	$successPayTransactionActive = true;
	...
	if(!$db->query("update player set yb=COALESCE(yb,0)+".intval($orderInfo['getyb'])." where id=".intval($orderInfo['user_id'])) ||
		mysql_affected_rows($db->getConn()) != 1){
		successPayAbort("Q_ERROR_".intval($orderInfo['user_id'])."_".$price);
	}
	if(!$db->query('COMMIT')) successPayAbort("Q_ERROR_".intval($orderInfo['user_id'])."_".$price);
	$successPayTransactionActive = false;
