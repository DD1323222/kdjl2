...
$err = 0;
$pendingPaiLog = false;
$sjPaiBuyTransactionActive = false;
$sjPaiBuyLockedItemId = 0;

function sjPaiBuyShutdown()
{
	global $_pm, $sjPaiBuyTransactionActive, $sjPaiBuyLockedItemId;
	$error = error_get_last();
	if(!is_array($error) || !in_array($error['type'], array(E_ERROR, E_PARSE, E_CORE_ERROR, E_COMPILE_ERROR), true)) return;
	if($sjPaiBuyTransactionActive)
	{
		$_pm['mysql']->query('ROLLBACK');
		$sjPaiBuyTransactionActive = false;
	}
	if($sjPaiBuyLockedItemId > 0)
	{
		unLockItem($sjPaiBuyLockedItemId);
		$sjPaiBuyLockedItemId = 0;
	}
}
register_shutdown_function('sjPaiBuyShutdown');

$user		 = $_pm['user']->getUserById($uid);
if(!is_array($user)) die('1');
$userDefaults = array('maxbag'=>0, 'nickname'=>'');
...
if(!$_pm['mysql']->query('START TRANSACTION')) { unLockItem($bid); $sjPaiBuyLockedItemId = 0; $_pm['mem']->memClose(); die('3'); }
$sjPaiBuyTransactionActive = true;
$paiProps = $_pm['mysql']->getOneRecord("SELECT *
								 FROM userbag
								WHERE id={$bid} and psj>0 and psum>0 and petime>{$now} FOR UPDATE
							 ");
...
$paiProps['psj'] = intval($paiProps['psj']);
...
$priceSum = kdjlSafePositiveProduct($paiProps['psj'], $n);
if ($priceSum === false)
{
	unLockItem($bid);
	$_pm['mysql']->query('ROLLBACK');
...
/****打开MYSQL事务，禁止自动提交****/
		...
		if(!$_pm['mysql']->query("UPDATE userbag
							     SET uid={$uid},
									 ...
									 buycode=0,
									 sums=1
							   WHERE psj>0 and psum>0 and id={$bid} and uid={$paiProps['uid']} and vary=2") ||
...
/**************提交事务*************/
       // $_pm['mysql']->query('COMMIT');
		...
		if(!savelog($paiProps['uid'], $uid, '交易物品：'.$bid.' 1个.'.$logsql, 102))
		{
			unLockItem($bid);$_pm['mysql']->query('ROLLBACK');
			die('3');
		}
		...
		if(!empty($paiProps['buycode']))
		{
			if($paiProps['psum'] > $n){
				$updateOk = $_pm['mysql']->query("UPDATE userbag
							   ...
							   WHERE psj>0 and psum>{$n} and id={$bid} and uid={$paiProps['uid']} and vary=1
							");
			}else{
				$updateOk = $_pm['mysql']->query("UPDATE userbag
								 SET psum=0,buycode=0,psell = 0,psj = 0,pyb = 0,pstime=0,petime=0
							   WHERE psj>0 and psum={$n} and id={$bid} and uid={$paiProps['uid']} and vary=1
							");
			}
...
/**************提交事务*************/
			...
			if(!savelog($paiProps['uid'], $uid, '交易物品：'.$bid.'物品ID：'.$pid.', '.$n.'个;'.$logsql, 102))
			{
				unLockItem($bid);$_pm['mysql']->query('ROLLBACK');
				die('3');
			}
...
function updateUser($selluid,$buyuid,$priceSum)
{
	...
	$newPriceSum = intval(round($priceSum * 0.92));
/****打开MYSQL事务，禁止自动提交****/
	$payOk = $_pm['mysql']->query("UPDATE player_ext
							 SET sj=sj-{$priceSum}
	...
	$settleOk = $_pm['mysql']->query("INSERT INTO player_ext(uid,bbshow,paisj)
							 VALUES({$selluid},5,{$newPriceSum})
							 ON DUPLICATE KEY UPDATE paisj=COALESCE(paisj,0)+{$newPriceSum}
						");
	if(!$settleOk || mysql_affected_rows($_pm['mysql'] -> getConn()) < 1){
		return false;
	}
	...
	$logsql .=";UPDATE player_ext
			   ...
			   WHERE uid={$buyuid}
				";
    $logsql .=";UPDATE player_ext
				 SET paisj=COALESCE(paisj,0)+{$newPriceSum}
			   WHERE uid={$selluid}
			  ";
	return true;
}
