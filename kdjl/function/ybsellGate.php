...
$err = 0;
$ybPaiSellTransactionActive = false;
$ybPaiSellLockedItemId = 0;

function ybPaiSellShutdown()
{
	global $_pm, $ybPaiSellTransactionActive, $ybPaiSellLockedItemId;
	$error = error_get_last();
	if(!is_array($error) || !in_array($error['type'], array(E_ERROR, E_PARSE, E_CORE_ERROR, E_COMPILE_ERROR), true)) return;
	if($ybPaiSellTransactionActive) $_pm['mysql']->query('ROLLBACK');
	$ybPaiSellTransactionActive = false;
	if($ybPaiSellLockedItemId > 0)
	{
		unLockItem($ybPaiSellLockedItemId);
		$ybPaiSellLockedItemId = 0;
	}
}
register_shutdown_function('ybPaiSellShutdown');
...
else
{
	...
	*/
	...
	if(is_array($userbag)) foreach ($userbag as $x => $y)
	{
		...
		if(!isset($y['pyb'])) $y['pyb'] = 0;
		if(!isset($y['psum'])) $y['psum'] = 0;
		if ($y['pyb']>0 && $y['psum']>0) $painum++;
		if ($painum > 3) {
			unLockItem($bid);
			die("4");
		}
	...
	{   // 是否可交易检查
		...
		if(!$_pm['mysql']->query('START TRANSACTION')) { unLockItem($bid); $ybPaiSellLockedItemId = 0; $_pm['mem']->memClose(); die('3'); }
		$ybPaiSellTransactionActive = true;
