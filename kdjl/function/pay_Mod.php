...
if(!isset($_SESSION['username'])||strlen($_SESSION['username'])<3){
	echo '<script language="javascript">alert("请先登陆!");window.location="http://gm.yibaoma.shop";</script>';exit();
}
if(isset($_SESSION['51_session']) && is_array($_SESSION['51_session'])){
	...
	echo '<script language="javascript">alert("登陆失效!");window.location="http://gm.yibaoma.shop";</script>';exit();
}
...
if(isset($_GET['num']) && !is_array($_GET['num']) && intval($_GET['num'])>0){
	...
	array(
		...
		'order_num'         => $price,
		'pay_shipping'      => 'http://'.$httpHost.'/function/success_Mod'.$houzhui.'.php',
		'order_msg'=>$msg,
		'order_callback_url'=> 'http://'.$httpHost.'/function/success_Mod'.$houzhui.'.php',//?order_code=0
		'order_check_url'   => 'http://'.$httpHost.'/function/success_Mod'.$houzhui.'.php', // 支付成功后的对帐通知地址，最多 200 字节。
		'order_cancel_url'  => 'http://'.$httpHost.'/function/success_Mod'.$houzhui.'.php?cancel=1'
	);
