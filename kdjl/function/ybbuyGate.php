...
$err = 0;
$pendingPaiLog = false;
$ybPaiBuyTransactionActive = false;
$ybPaiBuyLockedItemId = 0;

function ybPaiBuyShutdown()
{
	global $_pm, $ybPaiBuyTransactionActive, $ybPaiBuyLockedItemId;
	$error = error_get_last();
	if(!is_array($error) || !in_array($error['type'], array(E_ERROR, E_PARSE, E_CORE_ERROR, E_COMPILE_ERROR), true)) return;
	if($ybPaiBuyTransactionActive)
	{
		$_pm['mysql']->query('ROLLBACK');
		$ybPaiBuyTransactionActive = false;
	}
	if($ybPaiBuyLockedItemId > 0)
	{
		unLockItem($ybPaiBuyLockedItemId);
		$ybPaiBuyLockedItemId = 0;
	}
}
register_shutdown_function('ybPaiBuyShutdown');

$user		 = $_pm['user']->getUserById($uid);
if(!is_array($user)) die('1');
$userDefaults = array('yb'=>0, 'maxbag'=>0, 'nickname'=>'');
...
if(!$_pm['mysql']->query('START TRANSACTION')) { unLockItem($bid); $ybPaiBuyLockedItemId = 0; $_pm['mem']->memClose(); die('3'); }
$ybPaiBuyTransactionActive = true;
$paiProps = $_pm['mysql']->getOneRecord("SELECT *
								 FROM userbag
								WHERE id={$bid} and pyb>0 and psum>0 and petime>{$now} FOR UPDATE
							 ");
...
$paiProps['pyb'] = intval($paiProps['pyb']);
...
$priceSum = kdjlSafePositiveProduct($paiProps['pyb'], $n);
if ($priceSum === false)
{
	unLockItem($bid);
	$_pm['mysql']->query('ROLLBACK');
...
/****打开MYSQL事务，禁止自动提交****/
		...
		if(!$_pm['mysql']->query("UPDATE userbag
							     SET uid={$uid},
									 ...
									 sums=1,
									 pyb=0
							   WHERE pyb>0 and psum>0 and id={$bid} and uid={$paiProps['uid']} and vary=2") ||
...
/**************提交事务*************/
       // $_pm['mysql']->query('COMMIT');
		...
		if(!savelog($paiProps['uid'], $uid, '交易物品：'.$bid.' 1个.'.$logsql, 108))
		{
			unLockItem($bid);$_pm['mysql']->query('ROLLBACK');
			die('3');
		}
		...
		if(!empty($paiProps['buycode']))
		{
			if($paiProps['psum'] > $n){
				$updateOk = $_pm['mysql']->query("UPDATE userbag
							   ...
							   WHERE pyb>0 and psum>{$n} and id={$bid} and uid={$paiProps['uid']} and vary=1
							");
			}else{
				$updateOk = $_pm['mysql']->query("UPDATE userbag
...
/**************提交事务*************/
			...
			if(!savelog($paiProps['uid'], $uid, '交易物品：'.$bid.'物品ID：'.$pid.', '.$n.'个;'.$logsql, 108))
			{
				unLockItem($bid);$_pm['mysql']->query('ROLLBACK');
				die('3');
			}
...
function updateUser($selluid,$buyuid,$priceSum)
{
	...
	$newPriceSum = intval(round($priceSum * 0.95));
/****打开MYSQL事务，禁止自动提交****/
	...
	$settleOk = $_pm['mysql']->query("INSERT INTO player_ext(uid,bbshow,paiyb)
							 VALUES({$selluid},5,{$newPriceSum})
							 ON DUPLICATE KEY UPDATE paiyb=COALESCE(paiyb,0)+{$newPriceSum}
						");
    ...
    $logsql .=";UPDATE player_ext
				 SET paiyb=COALESCE(paiyb,0)+{$newPriceSum}
			   WHERE uid={$selluid}
			  ";
	return true;
}
