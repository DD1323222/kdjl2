...
$requestUri = (isset($_SERVER['REQUEST_URI']) && !is_array($_SERVER['REQUEST_URI'])) ? $_SERVER['REQUEST_URI'] : '/function/Fight_Mod_Mobile.php';
if($requestUri === '' || substr($requestUri, 0, 1) !== '/' || preg_match('/["\'\r\n]/', $requestUri)) $requestUri = '/function/Fight_Mod_Mobile.php';

function fightMobileJsSingle($value)
{
	return str_replace(array('\\', "'", "\r", "\n", '<', '>'), array('\\\\', "\\'", '', '', '\\x3C', '\\x3E'), strval($value));
}

function fightMobileImage($value)
{
...
else if($memmapid[$inmap]['multi_monsters']==3&&$teamId>0)
{
	...
	foreach($teamInfo['members'] as $__k=>$mem)
	{
		if(isset($mem['state']) && intval($mem['state'])==1)
		{
			...
			{
				die('<script language="javascript">parent.Alert("队伍中有队员的主战宠物等级低于'.$minLevel.'级！");window.location="/function/Team_Mod.php?n='.$_SESSION['team_inmap'].'";</script>');
...
window.location="/function/Team_Mod.php?n='.$_SESSION['team_inmap'].'";
			...
			if(!$needRow&&$userbb['uid']>0&&substr($userbb['lttime'],9)<10)
			{
				if(isset($_GET['oksj']))
				{
					...
					//$s=$team->getTeamState();
                    $sr=$team->snotice(kdjlSafeIconv('gbk','utf-8','_AL_队伍中'.$userbb['nickname'].'今日免费次数已用完，继续战斗需要扣除队长'.$sj.'水晶！'),$teamInfo,$exclude);



                    die('<script language="javascript">parent.Alert("队伍中'.$userbbNicknameJs.'今日免费次数已用完，继续战斗需要扣除队长'.$sj.'水晶！");window.location="/function/Team_Mod.php?n='.$_SESSION['team_inmap'].'";</script>');
...
if($teamId>0)
{
	...
	foreach($teamInfo['members'] as $mem)
	{
		...
		else if($theTeamFubenMap!==false&&$memberState<1&&isset($mem['uid'])) { // kick inactive team fuben member
...
}else if($maparr['multi_monsters'] == 2){ // tower map
	$_SESSION['multi_monsters'.$uid] = 3;
}else{
	$_SESSION['multi_monsters'.$uid] = 2; // normal map
}
...
{   if (pt==1) return;
	if(m == 0){
		window.parent.document.getElementById("gw").src="./function/Fight_Mod_Mobile.php?p='.$waitBid.'&s=t";
	}
	pt=1;
 }
loadtime('.$will.');
...
else
{
   ...
   // ================================ 装备效果结束 ========================================
	...
	// By field order.
	$bb['wx'] = getWx($bb['wx']);
	$bbNameJs = fightMobileJsSingle($bb['name']);
	$bbWxJs = fightMobileJsSingle($bb['wx']);
	$bbSkillJs = fightMobileJsSingle($bb['skillist']);
	$bbImgStand = fightMobileImage($bb['imgstand']);
	$bbImgAck = fightMobileImage($bb['imgack']);
	$bbImgDie = fightMobileImage($bb['imgdie']);
...
if (!is_array($bjn)){
	header("refresh:2;url=Fight_Mod_Mobile.php?p={$bid}");
...
foreach($bjn as $k => $rs)
{
	   ...
	   )
	{
		...
		$skillNameJs = fightMobileJsSingle($rs['name']);
		$skillValueJs = fightMobileJsSingle($rs['value']);
		$skillPlusJs = fightMobileJsSingle($rs['plus']);
		$skillImg = fightMobileImage($rs['img']);
...
if($_SESSION['multi_monsters'.$_SESSION['id']] == 1){//挑战
	...
	//$gw = $memgpc[$gw1[0]['gid']];
	$gw = getBaseGpcInfoById($gw1[0]['gid']); // 单条读取怪物记录
	...
	//$gw = $memgpc[$gw1[0]['gid']];
	$gw = getBaseGpcInfoById($gw1[0]['gid']); // 单条读取怪物记录
	if($tgcheck['tgt'] == 30){ // charge 200 sj
		...
		if($tg31check != 1 && (!isset($_GET['confirm31']) || is_array($_GET['confirm31']) || $_GET['confirm31'] != 'yes')){
			die('<script language="javascript">if(confirm("继续31层，将收取200水晶，是否继续？")){
				window.setTimeout("window.parent.$(\"gw\").src=\"function/Fight_Mod_Mobile.php?p='.$waitBid.'&confirm31=yes\"",1000);
			}else{
				...
				if($theTeamFubenMap!==false)
				{
					...
					*/
					...
					)
					{
						...
						if(!$gw)
						{
							die('找不到副本怪物配置，地图：'.$theTeamFubenMap['id'].'，阶段：'.$teamState_team_fuben_step.'，组：'.$teamState_team_fuben_step1);
						}
						...
						{
						$tempGpcInfo = getBaseGpcInfoById($gid); // 单条读取怪物记录
	...
	";
				...
				if(strpos($teamState['fight_html'],'<body')===false&&strlen($teamState['fight_html'])<100)
				{
					...
					if($_SESSION['waitTeamTime']>20)
					{
						...
						$_pm['mem']->setns('pm_team_fight_'.$teamId,$dataNow);
						$team->clearTeamState();
						header('location:/function/Fight_Mod_Mobile.php?'.($teamState['autofighting']?'team_auto=1':'').'&type=1');
					...
					{
						echo '<br/><span style="font-size:12px" onclick="window.parent.$(\'gw\').src=\'./function/Fight_Mod_Mobile.php?&type=1\';">有队员相应超时,点击这里,重新开始战斗!</font>';
					}
					exit();
					//$team->clearTeamState();
				}else{
...
$_SESSION['gwcdie'.$_SESSION['id']] = $gw['id'];
	$gwNameJs = fightMobileJsSingle($gw['name']);
	$gwWxJs = fightMobileJsSingle($gw['wx']);
	$gwSkillJs = fightMobileJsSingle($gw['skill']);
	$gwImgStand = fightMobileImage($gw['imgstand']);
	$gwImgAck = fightMobileImage($gw['imgack']);
	$gwImgDie = fightMobileImage($gw['imgdie']);
